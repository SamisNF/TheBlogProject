...
function AddTag() {
    ...
    if (searchResult != null) {
        //Trigger my sweet alert for the empty string or whatever condition is contained in the searchResult variable.
    ...
    //Clear out the TagEntry control value.
    tagEntry.value = "";
    return true;
}
...
//The Search function will detect either an empty or a duplicate Tag (within a single post record)
    ...
    if (tagsEl) {
        ...
        for (let index = 0; index < options.length; index++) {
            if (options[index].value == str)
                return `Error. The Tag #${str} was detected as a duplicate. Duplicates are not allowed.`;
        }
    }
